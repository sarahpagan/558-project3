...
Logistic regression models are classification models estimating the probability of an event occurring given a set of predictors. The logistic regression function models an outcome as the log-odds of success:

$log(\frac{P(success|x)}{1-P(sucess|x)}) = \beta_0 + \beta_1x_1$

Therefore, the $\beta_1$ coefficient should be interpreted as the change in log-odds given a one-unit  change in x. After taking the exponent of both sides in the above equation, it follows that:

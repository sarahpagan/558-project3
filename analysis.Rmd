...
```{r}
logfit <- train(Diabetes ~ Age + HighChol + HighBP,
                 data = train_data,
                 method = "glm",
                 family = "binomial",
...
```{r}
logfit3 <- train(Diabetes ~ Veggies + Fruits + HvyAlcoholConsump + Smoker,
                 data = train_data,
                 method = "glm",
                 family = "binomial",
                 trControl = trainControl(method = "cv",
...
```{r}
log_results <- data_frame("Metric" = "logLoss",
                          "Model 1" = logfit$results$logLoss,
                          "Model 2" = logfit2$results$logLoss,
                          "Model 3" = logfit3$results$logLoss)
log_results
```
